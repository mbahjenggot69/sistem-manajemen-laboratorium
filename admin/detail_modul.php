...
if (!isset($_SESSION['log']) || $_SESSION['role'] !== 'admin') {
    ...
    $id_modl = isset($_GET['id_modl']) ? intval($_GET['id_modl']) : null;
    $id_labo = isset($_GET['id_labo']) ? intval($_GET['id_labo']) : null;
    $id_faku = isset($_GET['id_faku']) ? intval($_GET['id_faku']) : null;

    if ($id_modl === null || $id_modl <= 0 || $id_labo === null || $id_labo <= 0 || $id_faku === null || $id_faku <= 0) {
        header("Location: detail_peralatan.php?id_modl=$id_modl&id_labo=$id_labo&id_faku=$id_faku");
        exit();
    }
    $ambildaritabel = mysqli_query($koneksi, "SELECT * FROM daftar_modul WHERE id_faku = $id_faku AND id_labo = $id_labo");
    if (mysqli_num_rows($ambildaritabel) > 0) {
        if ($ambildaritabel) {
            $data = mysqli_fetch_assoc($ambildaritabel);
            $nama_modl = $data['nama_modl'];
            $kode_modul = $data['kode_modl'];
        } else {
            header("Location: detail_peralatan.php?id_modl=$id_modl&id_labo=$id_labo&id_faku=$id_faku");
            exit();
        }
                                ...
                                <div class="card-body">
                                    <table id="datatablesSimple">
                                        ...
                                        <tbody>
                                            ...
                                            $ambildaritabel = mysqli_query($koneksi, "SELECT * FROM daftar_alat WHERE id_faku = '$id_faku' AND id_labo = $id_labo AND pemilik = '$username' AND kode_modl LIKE '%$kode_modul%'");
                                            $nomor = 1;
